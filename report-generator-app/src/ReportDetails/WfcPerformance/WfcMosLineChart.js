...
const WfcMosLineChart = ({ tc, city = 'Seattle', hideTitle = false, chartWidth = '50%' }) => {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                ...
                const response = await fetch(`/AnalyzeResults/${folderName}${city}/wfc_linechart_data/wfc_mos_statistics_${tc.toLowerCase()}.json`);
                if (!response.ok) {
    ...
    return (
        <div style={{ width: chartWidth, height: 250, marginTop: '5px' }}>
