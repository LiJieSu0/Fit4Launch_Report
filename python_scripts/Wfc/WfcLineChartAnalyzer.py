...
def determine_tc_from_filename(filename):
...
def calculate_wfc_statistics(file_paths, output_json_path="wfc_statistics.json"):
    """
    Reads MOS values from all CSV files provided, performs interval statistics,
    calculates percentages, and outputs the aggregated results to a single JSON file.
    TC is determined from filename.
    """
    if not file_paths:
        return
    ...
    for csv_file_path in file_paths:
        ...
        tc_name = determine_tc_from_filename(filename)
        tc_num = None
        if tc_name:
...
def calculate_wfc_rssi_statistics(file_paths, output_json_path="wfc_rssi_statistics.json"):
    """
    Reads WiFi RSSI values from all CSV files provided, performs interval statistics,
    calculates percentages, and outputs the aggregated results to a single JSON file.
    TC is determined from filename.
    """
    if not file_paths:
    ...
    scenario_samples = {
        "MO DUT": [], "MT DUT": [], "MO REF": [], "MT REF": [], "DUT": [], "REF": []
    }
